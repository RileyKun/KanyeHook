...
void CInterfaces::Init()
{
	...
	IPhysics = g_Interface.Get<i_physics*>(L"vphysics.dll", "VPhysics031");
	_valid(IPhysics);

	IPhyicsCollison = g_Interface.Get<i_physics_collision*>(L"vphysics.dll", "VPhysicsCollision007");
	_valid(IPhyicsCollison);

	IPhysicsSurfaceProps = g_Interface.Get<i_physics_surface_props*>(L"vphysics.dll", "VPhysicsSurfaceProps001");
	_valid(IPhysicsSurfaceProps);
