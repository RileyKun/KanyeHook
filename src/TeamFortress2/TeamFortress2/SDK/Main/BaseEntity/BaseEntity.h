...
inline auto getAddr() {
	static auto addr = g_Pattern.Find(L"client.dll", L"55 8B EC 83 EC 0C 56 57 8B F1 E8 ? ? ? ? 8B F8 85 FF 74 10 8B 17 8B CF 8B 92 ? ? ? ? FF D2");
	return addr;
}

inline auto hi()
{
	static auto ret = g_Pattern.Find(L"client.dll", L"55 8B EC 83 EC ? 56 8B F1 57 80 BE ? ? ? ? ? 0F 85");
	return ret;
}
...
public: //Netvars & conditions
	...
	M_OFFSETGET(PipebombType, int, 0x8FC)
		...
		NETVAR(m_afButtonForced, int, _("CBasePlayer"), _("m_afButtonForced"))
		NETVAR(m_bGamePaused, bool, _("CBasePlayer"), _("m_bGamePaused"))
		NETVAR(m_afButtonDisabled, int, _("CBasePlayer"), _("m_afButtonDisabled"))
		NETVAR(m_nImpulse, int, _("CBasePlayer"), _("m_nImpulse"))
		...
		NETVAR(m_iEFlags, int, _("CBasePlayer"), _("m_iEFlags"))
		...
		NETVAR(m_nSequence, int, _("DT_BaseAnimating"), _("m_nSequence"))
		NETVAR(m_flCycle, float, _("DT_ServerAnimationData"), _("m_flCycle"))
		NETVAR(m_flAnimTime, int, _("DT_AnimTimeMustBeFirst"), _("m_flAnimTime"))
		NETVAR(m_hOwnerEntity, int, _("CBaseAnimating"), _("m_hOwnerEntity"))
		NETVAR(m_ConditionList, void*, _("CTFPlayer"), _("m_ConditionList"))
		NETVAR(m_nForcedSkin, int, _("CTFPlayer"), _("m_nForcedSkin"));
		NETVAR(m_bForcedSkin, bool, _("CTFPlayer"), _("m_bForcedSkin"));
		NETVAR(m_nSkin, int, _("CBaseAnimating"), _("m_nSkin"));
		NETVAR(m_iPlayerSkinOverride, int, _("CTFPlayer"), _("m_iPlayerSkinOverride"));
		NETVAR(m_nDisguiseSkinOverride, int, _("CTFPlayer"), _("m_nDisguiseSkinOverride"));
		...
		NETVAR(Criticals, int, "CTFPlayer", "m_iCrits");
public: //Virtuals
...
public: //Everything else, lol.
	...
	{
		static auto offset{ g_NetVars.get_offset("CBaseEntity", "moveparent") - 4 };
	...
	float FrameAdvance(float flInterval) {
		return reinterpret_cast<float(__thiscall*)(void*, float)>(hi())(this, flInterval);
	}

	void UpdateAnim()
	{
		static auto FN = reinterpret_cast<void(__thiscall*)(void*)>(getAddr());
		return FN(this);
	}
	...
	__inline void SetPoseParam(std::array<float, 24> param) {
		static DWORD dwOffset = g_NetVars.get_offset(_("DT_BaseAnimating"), _("m_flPoseParameter"));
		*reinterpret_cast<std::array<float, 24>*>(this + dwOffset) = param;
	}

	__inline std::array<float, 24> GetPoseParam() {
		static DWORD dwOffset = g_NetVars.get_offset(_("DT_BaseAnimating"), _("m_flPoseParameter"));
		return *reinterpret_cast<std::array<float, 24>*>(this + dwOffset);
	}
	...
	__inline void modify_eye_pos(Vec3* pos)
	{
		const auto& animstate = GetAnimState();

		if (m_flFallVelocity() <= 0 /// in hitground
			|| m_flDucktime() != 0.f
			|| !GetGroundEntity())
		{
			auto base_entity = animstate->m_pPlayer;

			auto bone_pos = GetBonePos(HITBOX_HEAD);
	...
	__inline Vec3 GetShootPos() {
		...
		//	Weapon_ShootPositionFn(this, &ret);
		//	return ret;
	//	}
