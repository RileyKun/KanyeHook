...
namespace Colors
{
	...
	inline Color_t TeamRed =					{ 184, 56, 59, 255 };
	inline Color_t TeamBlu =					{ 88, 133, 162, 255 };
...
namespace Utils
{
	...
	__inline float SharedRandomFloat(const char* sharedname, float flMinVal, float flMaxVal, int additionalSeed)
	{
		int seed = SeedFileLineHash(G::LastUserCmd->random_seed, sharedname, additionalSeed);
		RandomSeed(seed);
	...
	__inline bool VisPosHull(CBaseEntity* pSkip, CBaseEntity* pEntity, const Vec3& from, const Vec3& to, const Vec3& min, const Vec3& max)
	{
		CGameTrace trace = {};
		CTraceFilterHitscan filter = {};
		filter.pSkip = pSkip;
		TraceHull(from, to, min, max, (MASK_SHOT | CONTENTS_GRATE), &filter, &trace);
		return ((trace.entity && trace.entity == pEntity) || trace.flFraction > 0.99f);
	}
	...
	__inline bool IsAttacking(CUserCmd* pCmd, CBaseCombatWeapon* pWeapon)
	{
		if (pWeapon->GetSlot() == SLOT_MELEE)
		{
			...
			{
				return (pCmd->buttons & IN_ATTACK && pWeapon->GetNextPrimaryAttack());
			}
		...
		else
		{
			...
			else
			{
				...
				case TF_WEAPON_FLAME_BALL:
				{
					if ((pCmd->buttons & IN_ATTACK2) && G::WeaponCanSecondaryAttack)
					{
						return true;
					}
	...
	__inline void StopMovement(CUserCmd* pCmd, bool Safe = true) {
		if (Safe && G::IsAttacking && G::faggot) { return; }
		...
		if (CBaseEntity* pLocal = g_EntityCache.GetLocal()) {
			...
			dir.y = I::EngineClient->GetViewAngles().y - dir.y;
			Vec3 forward{};
